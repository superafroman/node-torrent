...
function DefaultRequestManager(bitfield, pieces) {
  ...
  pieces.forEach(function(piece) {
    piece.on(Piece.COMPLETE, requestManager.pieceComplete.bind(requestManager, piece));
  });
}
...
DefaultRequestManager.prototype.pieceComplete = function(piece) {
  LOGGER.debug('pieceComplete: ' + piece.index);
  this._bitfield.set(piece.index);
  LOGGER.debug('pieceComplete: bitfield cardinality ' + this._bitfield.cardinality());
};
