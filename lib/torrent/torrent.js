...
Torrent.prototype.addPeer = function(/* peer | id, address, port */) {
  ...
  if (!(peer.getIdentifier() in this.peers)) {
    ...
    peer.on(Peer.EXTENDED, function(key, message) {
      LOGGER.debug('EXTENDED from %s, key = %s', peer.getIdentifier(). key);
      if (torrent._extensionMap[key]) {
        torrent._extensions[torrent._extensionMap[key] - 1].handleMessage(peer, message);
      }
    });
