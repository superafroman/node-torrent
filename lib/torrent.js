...
Torrent.prototype.addPeer = function(peer) {
  ...
  peer.on(Peer.UPDATED, function() {
    var interested = self.bitfield.and(peer.bitfield).setIndexes().length > 0;
...
Torrent.prototype.removePeer = function(peer) {
  ...
  // check here just in case we're removing a duplicate
  if (this.peers[peer.peerId] && !this.peers[peer.peerId].initialised) {
...
function peerReady(self, peer) {
  if (!peer.active && 
        self.activePeers >= MAX_ACTIVE_PEERS) {
    return;
  }
  var piece;
...
function selectPeer(self) {
  if (self.activePeers < MAX_ACTIVE_PEERS) {
    // TODO: some sort of nifty algorithm
