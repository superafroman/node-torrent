...
// calculate average upload/download rate over the last 10 secs
function updateRates(self, kind) {
  var history = (kind=='up') ? self.uploadedHistory : self.downloadedHistory;
  var rates = (kind=='up') ? self.uploadRates : self.downloadRates;

  var now = Date.now();
  history.push({ts: now, value: self.downloaded});
  if (history.length > 1) {
    var start = history[0].ts;
    if (now-start > 1*1000) {
      // calculate a new rate at the same time clearing the history
      rates.push({ts: now, value: (history.pop().value-history.pop().value)/(now-start)*1000});
  ...
  var decayRate = 0.13863;
  var rateSum = 0, weightSum = 0;
  for (var idx=0; idx<rates.length; idx++) {
    var age = rates[idx].ts-rates[0].ts;
    var weight = Math.exp(-decayRate*age/1000);
    rateSum += rates[idx].value * weight;
    weightSum += weight;
  }

  if (kind=='up') {
    self.currentUploadRate = rateSum / weightSum;
  }
  else {
    self.currentDownloadRate = rateSum / weightSum;
  }
}
