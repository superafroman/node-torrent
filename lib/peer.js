...
Peer.prototype.requestPiece = function(piece) {
  if (this.numRequests < MAX_REQUESTS) {
    ...
    console.log('Peer.requestPiece, nextChunk = ', nextChunk);
    if (nextChunk) {
      ...
      this.requests[piece.index][nextChunk.begin] = true;
...
Peer.prototype.sendMessage = function(message) {
  console.log('Peer.sendMessage, code = ', message.code);
...
function handleHandshake(self) {
  console.log('Peer.handleHandshake');
  var data = self.data;
  if (data.length < 68) {
    // Not enough data.
    return;
...
function processData(self) {
  ...
  if (messageLength === 0) {
    ...
    console.log('Peer.processData, code = ', code, ' message length = ', messageLength);
