...
Peer.prototype.disconnect = function(message, reconnectTimeout) {
  ...
  for (var index in this.pieces) {
    ...
    if (requests) {
      ...
      this.requests[index] = {};
      delete this.requestsCount[index];
    }
  }
...
function onData(self, data) {
  ...
  //console.log('appended ' + data.length + ' bytes, have ' + self.data.length + ' bytes');
  if (!self.initialised) {
            ...
            else if (self.data.length-offset >= messageLength)
            {
                ...
                switch (message.code) {
                  ...
                  case Message.REQUEST:
                    ...
                    // need to wait for requestChunk to callback, exit next/do-while but preserve state
                    return;
                  ...
                  case Message.PIECE:
                    ...
                    if (piece) {
                        // setData returns true if the caller should wait for callback
                        if (piece.setData(data, begin, function(err) {
                            // only get a callback if piece is complete and validates ok
                            if (err) {
                              LOGGER.warn('Failed to write to disk: ' + err);
                            }
                            ProcessUtils.nextTick(next);
                          }))
                        {
                          // we are waiting for callback, exit next/do-while loop but preserve state
                          return;
                        }
                        else
                        {
                          if (self.requestsCount[piece.index] < 5) {
                            ProcessUtils.nextTick(function() {
...
function updateRates(self, kind) {
  ...
  if (history.length > 1) {
    ...
    if (now-start > 1*1000) {
      ...
      while((rates.length>1) && (now-rates[0].ts>10*1000)) {
        rates.shift();
      }
