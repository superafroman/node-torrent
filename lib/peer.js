...
function handleHandshake(self) {
  ...
  if (!BufferUtils.equal(BITTORRENT_HEADER.slice(0, 20), data.slice(0, 20))) {
    ...
    self.supportsExtension = data[25] & 0x10;
    console.log('Peer supports extension: ', self.supportsExtension);

    if (self.supportsExtension) {
      self.sendMessage(new Message(Message.EXTENDED, BufferUtils.fromInt(0)));
    }
          ...
          else if (self.data.length-offset >= messageLength)
          {
              ...
              switch (message.code) {
                ...
                case Message.UNCHOKE:
                  LOGGER.debug('Received UNCHOKE from ' + Peer.getIdentifier(self));
                  self.choked = false;
                  if (self.amInterested) {
                    self.emit(Peer.READY);
                  }
                ...
                case Message.EXTENDED:
                  ...
                  var extensions = bencode.decode(message.payload.toString());
                  if (extensions.m['ut_metadata']) {
                    self.hasMetadata = true;
                    self.metadataMessageCode = extensions.m['ut_metadata'];
                    self.metadataSize = extensions['metadata_size'];
                  }
                  break;
