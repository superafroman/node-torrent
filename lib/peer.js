...
Peer.prototype.setAmInterested = function(interested) {
  ...
  if (interested && !self.amInterested) {
    ...
    LOGGER.debug('Sent INTERESTED to ' + self.stream.remoteAddress + ':' + self.stream.remotePort);
    ...
    LOGGER.debug('Sent UNINTERESTED to ' + self.stream.remoteAddress + ':' + self.stream.remotePort);
...
Peer.prototype.setTorrent = function(torrent) {
  ...
  if (this.stream && !this.initialised) {
    ...
    LOGGER.debug('Sent BITFIELD to ' + stream.remoteAddress + ':' + stream.remotePort);
  }
};

Peer.prototype.supportsExtension = function(key) {
  if (key && this._extensionData) {
    return this._extensionData.m[key];
  }
...
function doHandshake(self) {
  ...
  this.handshake = true;
  LOGGER.debug('Sent HANDSHAKE to ' + stream.remoteAddress + ':' + stream.remotePort);
}
...
function handleHandshake(self) {
  ...
  if (!BufferUtils.equal(BITTORRENT_HEADER.slice(0, 20), data.slice(0, 20))) {
    ...
    LOGGER.debug('Received HANDSHAKE from ' + self.stream.remoteAddress + ':' + self.stream.remotePort);
              ...
              switch (message.code) {
                ...
                case Message.EXTENDED:
                  ...
                  if (extendedCode === 0) { // handshake
                    ...
                    var extensionKey;
                    Object.keys(self._extensionData.m).some(function(key) {
                      if (self._extensionData.m[key] === extendedCode) {
                        extensionKey = key;
                        return true;
                      }
                    });
                    LOGGER.debug('Peer [%s] received extended code %d', self.getIdentifier(), extendedCode);
                    self.emit(Peer.EXTENDED, self, extensionKey, data);
                  }
                  break;
