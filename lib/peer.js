...
Peer.prototype.sendExtendedMessage = function(code, data) {
  ...
  if (this.extensions[code] !== undefined) {
    var payload = bencode.encode(data);
    var message = new Message(this.extensions[code], new Buffer(payload));
    this.sendMessage(message);
    if (!this.running) {
      this.running = true;
      var peer = this;
      ProcessUtils.nextTick(function(){nextMessage(peer);});
    }
  } else {
    // throw?
          ...
          else if (self.data.length-offset >= messageLength)
          {
              ...
              switch (message.code) {
                ...
                case Message.BITFIELD:
                  ...
                  self.emit(Peer.UPDATED);
                  if (!self.torrent.bitfield) {
                    console.log('sending bitfield back');
                    self.sendMessage(new Message(Message.BITFIELD, new Buffer(self.bitfield.length / 8))); 
                  }
                  break;
                  ...
                  if (extendedCode === 0) {
                    ...
                    Object.keys(self.extensions).some(function(key) {
                      if (self.extensions[key] === extendedCode) {
                        self.emit(Peer.EXTENDED, self, key, payload);
                        return true;
                      }
                    });
                  }
