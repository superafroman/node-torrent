...
Peer.prototype.requestPiece = function(piece) {
  ...
  if (!piece.isComplete()) {
    ...
    payload = BufferUtils.concat(payload, BufferUtils.fromInt(piece.nextChunkStart()));
  ...
  if (messageLength === 0) {
    ...
    switch (message.code) {
      ...
      case Message.HAVE:
        ...
        var index = Math.floor(piece / 8);
        var bit = piece % 8;
        self.bitfield[index] = self.bitfield[index] | Math.pow(2, bit);
        if (!self.downloading) {
          self.emit(READY);
        }
        break;
