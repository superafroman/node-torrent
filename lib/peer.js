...
Peer.prototype.requestPiece = function(piece) {
  ...
  if (!piece.isComplete()) {
    ...
    console.log('piece incomplete, requesting next chunk, index =', piece.index, 'begin =', piece.nextChunkBegin());
    var message = new Message(Message.REQUEST, payload);
    this.sendMessage(message);
  } else {
    console.log('piece complete, index =', piece.index);
    this.piece = null;
    this.emit(COMPLETE, piece);
  }
};
