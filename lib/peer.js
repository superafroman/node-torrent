...
Peer.prototype.disconnect = function(message) {
  LOGGER.info('Peer.disconnect message =', message);
  this.disconnected = true;
  if (this.keepAliveId) {
	  LOGGER.debug('clearing interval in disconnect');
...
function nextMessage(self) {
  if (!self.disconnected && self.initialised) {
    if (self.messages.length === 0) {
      ...
      if (self.keepAliveId) {
    	LOGGER.debug('clearing interval in nextMessage');
  ...
  if (messageLength === 0) {
    ...
    switch (message.code) {
      ...
      case Message.UNCHOKE:
        LOGGER.debug('Peer received UNCHOKE message.');
        self.choked = false;
      ...
      case Message.PIECE:
        ...
        if (self.requests[index] 
          ...
          LOGGER.debug('Peer [' + self.address + '] download rate = ' + (averageRate / 1024) + 'Kb/s');
        }
