      ...
      if (self.data.length-offset >= 4)
      {
          ...
          else if (self.data.length-offset >= messageLength)
          {
              ...
              switch (message.code) {
                ...
                case Message.CHOKE:
                  LOGGER.info('Received CHOKE ' + Peer.getIdentifier(self));
                  self.choked = true;
                  self.emit(Peer.CHOKED);
                  break;

                case Message.UNCHOKE:
                  LOGGER.info('Received UNCHOKE ' + Peer.getIdentifier(self));
                  self.choked = false;
                  if (self.amInterested) {
                    self.emit(Peer.READY);
                  }
                ...
                case Message.INTERESTED:
                  LOGGER.info('Received INTERESTED ' + Peer.getIdentifier(self));
                  self.interested = true;
                  break;

                case Message.UNINTERESTED:
                  LOGGER.info('Received UNINTERESTED ' + Peer.getIdentifier(self));
                  self.interested = false;
                  break;

                case Message.HAVE:
                  LOGGER.debug('Received HAVE ' + Peer.getIdentifier(self));
                ...
                case Message.BITFIELD:
                  LOGGER.info('Received BITFIELD ' + Peer.getIdentifier(self));
                ...
                case Message.REQUEST:
                  LOGGER.debug('Received REQUEST ' + Peer.getIdentifier(self));
                ...
                case Message.PIECE:
                  //LOGGER.debug('Received PIECE ' + Peer.getIdentifier(self));
