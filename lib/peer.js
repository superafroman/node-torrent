...
function processData(self) {
  ...
  (function next() {
    if (self.data.length < 4) {
      LOGGER.debug('Not enough data ' + self.getIdentifier());
    ...
    if (messageLength === 0) {
      // Keep alive
      LOGGER.info('Sent keep alive to ' + self.getIdentifier());
      ...
      switch (message.code) {
        case Message.CHOKE:
          LOGGER.info('Sent CHOKE to ' + Peer.getIdentifier(self));
          self.choked = true;
          self.emit(Peer.CHOKED);
          break;
        case Message.UNCHOKE:
          LOGGER.info('Sent UNCHOKE to ' + Peer.getIdentifier(self));
          self.choked = false;
          if (self.amInterested) {
            self.emit(Peer.READY);
          }
        ...
        case Message.INTERESTED:
          LOGGER.info('Sent INTERESTED to ' + Peer.getIdentifier(self));
          self.interested = true;
          break;
        case Message.UNINTERESTED:
          LOGGER.info('Sent UNINTERESTED to ' + Peer.getIdentifier(self));
          self.interested = false;
          break;
        case Message.HAVE:
          LOGGER.debug('Sent HAVE to ' + Peer.getIdentifier(self));
          var piece = BufferUtils.readInt(message.payload);
          self.bitfield.set(piece);
          self.emit(Peer.UPDATED);
          break;
        case Message.BITFIELD:
          LOGGER.info('Sent BITFIELD to ' + Peer.getIdentifier(self));
        ...
        case Message.REQUEST:
          ...
          LOGGER.debug('Requested chunk from ' + Peer.getIdentifier(self) + ': index = ' + index + ', begin = ' + begin + ', length = ' + length);
        ...
        case Message.PIECE:
          ...
          if (self.requests[index]
            ...
            updateRates(self, 'down');
            LOGGER.debug('Downloading piece from ' + Peer.getIdentifier(self) + ', download rate = ' + (self.currentDownloadRate / 1024) + 'Kb/s');
          }
